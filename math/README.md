...
## 知识模块（15 个，覆盖初中三年课标主线）

| 领域 | 模块 | 核心实验 |
|---|---|---|
| 数与代数 | 负数与数轴（七） | 数轴移动游戏：加法是移动，减法是反向移动 |
| 数与代数 | 代数式与乘法公式（七~八） | 面积拼图：亲眼看到 (a+b)² 的四块、平方差的"切一刀拼过去" |
| 数与代数 | 方程：天平的语言（七） | 交互天平：两边同时操作解方程，理解"移项变号"的本质 |
| 数与代数 | 不等式（七） | 数轴上拖出解集"光带"；乘负数=镜像翻转的可视化 |
| 数与代数 | 方程组：鸡兔同笼（七） | 拖分界线凑 94 只脚，自己发现"抬脚法"就是消元 |
| 数与代数 | 平方根与无理数（七~八） | 夹逼 √2 永远差一点；用圆规把 √2 精确放上数轴（勾股客串） |
| 数与代数 | 一次函数（八） | k/b 旋钮 + 函数机器；手机套餐比价（交点=方程的解） |
| 数与代数 | 二次函数与抛物线（九） | 拖顶点玩抛物线；投篮小游戏（让轨迹穿过篮筐） |
| 图形与几何 | 三角形与平行线（七） | 直接拖顶点看内角和恒为 180°；平行线证明开关；多边形切三角形 |
| 图形与几何 | 勾股定理（八） | 三边上的正方形；四个三角形重排——亲手完成面积证明 |
| 图形与几何 | 相似（九） | 放大三角形看 k 与 k²；重演泰勒斯用影子量金字塔 |
| 图形与几何 | 锐角三角函数（九） | 拖角度看"边全变、比值不动"；用仰角不爬楼量楼高 |
| 图形与几何 | 圆与 π（九） | 内接多边形逼近 π（重走祖冲之的路）；圆周角定理探索 |
| 统计与概率 | 统计（八） | "富翁搬进小区"看平均数 vs 中位数；两位神枪手看方差 |
| 统计与概率 | 概率（八~九） | 抛一万次硬币亲眼看大数定律；两个骰子的"秘密形状" |

尚未覆盖、可作为后续扩展：全等三角形、四边形性质、平移/旋转/轴对称变换、反比例函数、因式分解专题、数据的收集与抽样。

## 功能

- **画布直接拖拽**：三角形顶点、抛物线顶点、圆上的点、数轴上的点、鸡兔分界线……都能直接用手指/鼠标抓着拖，不隔着滑杆（滑杆保留作为备选）。
